...
custom_imports = dict(
    imports=['mmdet3d.engine.hooks.ros_publish_hook'],
    allow_failed_imports=False)

data_root = '/kitti_data/KittiData'
...
default_hooks = dict(
    ...
    # ros_publish=dict(
    #     type='Det3DRosPublishHook',
    #     topic='/box',
    #     frame_id='velodyne',
    #     score_thr=0.1))
custom_hooks = [dict(type='Det3DRosPublishHook', 
                topic='/box', frame_id='velodyne',pred_score_thr=0.1)]
...
launcher = 'none'
load_from =  '/src/mmdetection_3d/src/mmdetection3d/mvxnet_dir/epoch_40.pth'
log_level = 'INFO'
...
test_dataloader = dict(
    ...
    dataset=dict(
        ...
        box_type_3d='LiDAR',
        data_prefix=dict(
            img='testing/image_2', pts='testing/velodyne_reduced'),
        data_root='/kitti_data/KittiData',
        metainfo=dict(classes=[
            'Pedestrian',
            'Cyclist',
...
test_evaluator = dict(
    ann_file='/kitti_data/KittiData/kitti_infos_test.pkl',
    type='KittiMetric',
    format_only=True,
    metric=[],
    submission_prefix='./mvxnet_test_work_dir/mvxnet_results_test')
test_pipeline = [
    dict(
        backend_args=None,
        coord_type='LIDAR',
...
val_dataloader = dict(
    ...
    sampler=dict(shuffle=False, type='DefaultSampler'))
val_evaluator = dict(
    ann_file='/kitti_data/KittiData/kitti_infos_val.pkl', type='KittiMetric')
vis_backends = [
